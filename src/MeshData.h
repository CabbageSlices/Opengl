 ...
 * A face is composed of 3 indices, 1 for each vertex. The index will simultaneously index into the postiion, normal, and texcoord array.
 * So a face with indices 0, 1, 2 means it uses position/normal/texcoord [0], [1], and [2]
 * All mesh attribtue data is stored in the MeshAttributes object.
 * If the texcoord or normal array is empty then the mesh does not contain any texcoord or normal data, only position
 * It's up to the user to make sure they don't utilize the mepty materials.*/
struct MeshData {
