...
void MeshRenderer::initializeVertexArrayObject() {
	...
	//even though all attributes are in the same buffer, you can still bind different sections of the buffer to different binding points
	//this allows texcoords to be stored in the same buffer even though it is jujst a vec2 instead of a vec3 like position and normal
	...
	//that way if different attributes have different number of elements you'll still be able to send the attirbute correctly
	...
	//that is, for vertex normals the buffer is bound at position POSITION_BUFFER_SIZE, that way the offset is already incorporated into the buffer itself
	//so no need to pass offset for this
	vao.formatVertexAttributeData(POSITION_ATTRIBUTE_LOCATION, 3, GL_FLOAT, 0, GL_FALSE);
	vao.formatVertexAttributeData(NORMAL_ATTRIBUTE_LOCATION, 3, GL_FLOAT, 0, GL_FALSE);

	
	//no tex coords to send
	if(meshData->attributes.texCoords.size() == 0)
		return; 
	...
	vao.bindVertexAttributeBuffer(TEXCOORD_BUFFER_BINDING_INDEX, attributeBuffer, positionBufferSize + normalBufferSize, sizeof(glm::vec2));

	vao.vertexAttributeShaderLocationToBufferIndex(TEXCOORD_ATTRIBUTE_LOCATION, TEXCOORD_BUFFER_BINDING_INDEX);
	vao.formatVertexAttributeData(TEXCOORD_ATTRIBUTE_LOCATION, 2, GL_FLOAT, 0);
